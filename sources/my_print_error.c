...
** Last update Sun Mar 19 20:58:08 2017 Antonin Rapini
*/

#include "utils.h"

void my_print_error(char *name)
{
  my_puterror("Usage : ");
  my_puterror(name);
  my_puterror(" [options]\n");
  ...
  my_puterror("\tMove the tetrimino RIGHT using the K key (def: right arrow)\n");
  ...
  my_puterror("Set the numbers of rows and columns of the map (def: 20, 10)\n");
